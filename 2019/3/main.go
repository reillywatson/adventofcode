...
func main_part1() {
	visited := map[[2]int]map[int]bool{}
	for i, wire := range strings.Split(input, "\n") {
		x := 0
		y := 0
	...
	var closest [2]int
	minDist := 100000000
	for coords, wires := range visited {
		if len(wires) == 2 {
			if dist(coords[0], coords[1]) < minDist {
				closest = coords
				minDist = dist(coords[0], coords[1])
			}
		}
	}
	fmt.Println(closest, minDist)
}
...
func main() {
	...
	for i, wire := range strings.Split(input, "\n") {
		...
		for _, path := range strings.Split(wire, ",") {
			...
			for j := 0; j < length; j++ {
				...
				coords := [2]int{x, y}
				if visited[coords] == nil {
					visited[coords] = map[int]int{}
				}
				if visited[coords][i] == 0 {
					visited[coords][i] = steps
				}
			}
		}
	}
	var closest [2]int
	minSteps := 1000000000
	for coords, steps := range visited {
		if len(steps) == 2 {
			if steps[0]+steps[1] < minSteps {
				minSteps = steps[0] + steps[1]
				closest = coords
			}
		}
	}
	fmt.Println(closest, minSteps)
}
