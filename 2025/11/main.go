...
const requiredMask = maskFFT | maskDAC
...
func main() {
	...
	for _, line := range strings.Split(input, "\n") {
		...
		deviceName := parts[0]
		connectionNames := strings.Split(parts[1], " ")
		devices[deviceName] = connectionNames
		if _, exists := nodesByName[deviceName]; !exists {
			nodesByName[deviceName] = &node{name: deviceName}
		}
		for _, connName := range connectionNames {
			if connName == "" {
				continue
			}
			if _, exists := nodesByName[connName]; !exists {
				nodesByName[connName] = &node{name: connName}
			}
		}
	}
	if _, exists := nodesByName["out"]; !exists {
		nodesByName["out"] = &node{name: "out"}
	}
	reverse := map[string][]string{}
	for deviceName, connectionNames := range devices {
		deviceNode := nodesByName[deviceName]
		for _, connName := range connectionNames {
			if connName == "" {
				continue
			}
			connNode, ok := nodesByName[connName]
			if !ok {
				continue
			}
			deviceNode.connections = append(deviceNode.connections, connNode)
			reverse[connName] = append(reverse[connName], deviceName)
		}
	}
	startNode, ok := nodesByName["svr"]
	if !ok {
		fmt.Println("start node 'svr' not found")
		return
	}
	reachableFromStart := bfsForward(startNode)
	canReachOut := bfsBackward("out", reverse)
	useful := map[string]bool{}
	for name := range reachableFromStart {
		if canReachOut[name] {
			useful[name] = true
		}
	}
	...
	if !useful[startNode.name] {
		fmt.Println("Number of paths: 0")
		return
	}
	order, ok := topologicalOrder(nodesByName, useful)
	if !ok {
		fmt.Println("cycle detected in reachable subgraph; cannot count paths with memoization")
		return
	}
	counts := make(map[string][4]uint64, len(order))
	for i := len(order) - 1; i >= 0; i-- {
		...
		for _, conn := range nodesByName[name].connections {
			if !useful[conn.name] {
				continue
			}
			childCounts := counts[conn.name]
			for maskIdx, cnt := range childCounts {
				if cnt == 0 {
					continue
				}
				combinedMask := nodeMask | maskIdx
				result[combinedMask] += cnt
			}
		}
		counts[name] = result
	}
	var total uint64
	for maskIdx, cnt := range counts[startNode.name] {
		if maskIdx&requiredMask == requiredMask {
			total += cnt
		}
	}
	fmt.Println(total)
...
func bfsForward(start *node) map[string]bool {
	...
	for len(queue) > 0 {
		...
		for _, conn := range current.connections {
			if visited[conn.name] {
				continue
			}
			visited[conn.name] = true
...
func bfsBackward(target string, reverse map[string][]string) map[string]bool {
...
func topologicalOrder(nodes map[string]*node, useful map[string]bool) ([]string, bool) {
	indegree := map[string]int{}
	for name := range useful {
		indegree[name] = 0
	}
	for name := range useful {
		for _, conn := range nodes[name].connections {
			if !useful[conn.name] {
				continue
			}
			indegree[conn.name]++
	...
	for len(queue) > 0 {
		...
		for _, conn := range nodes[current].connections {
			if !useful[conn.name] {
				continue
			}
			indegree[conn.name]--
			if indegree[conn.name] == 0 {
				queue = append(queue, conn.name)
			}
		}
	}
	if len(order) != len(useful) {
		return nil, false
	}
	return order, true
}

var sampleInputPartTwo = `svr: aaa bbb
aaa: fft
fft: ccc
bbb: tty
tty: ccc
...
func main_partone() {
	...
	for _, line := range strings.Split(input, "\n") {
		...
		connections := strings.Split(strings.Split(line, ": ")[1], " ")
		devices[deviceName] = connections
		nodes = append(nodes, &node{name: deviceName})
	}
	nodes = append(nodes, &node{name: "out"})
	for _, n := range nodes {
		for _, connName := range devices[n.name] {
			for _, potentialConn := range nodes {
				if potentialConn.name == connName {
					n.connections = append(n.connections, potentialConn)
				}
			}
		}
	}
	...
	for len(nodeQueue) > 0 {
		...
		nodeQueue = append(nodeQueue, currentNode.connections...)
	}
	fmt.Println(numPaths)
}

var sampleInput = `aaa: you hhh
you: bbb ccc
bbb: ddd eee
ccc: ddd eee fff
ddd: ggg
