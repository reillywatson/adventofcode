...
func main() {
	bags := parseBags(fulldata)
	for _, bag := range bags {
		if bag.color == "shiny gold" {
			fmt.Println(totalBags(bags, bag))
		}
	}
}

func totalBags(bags []bag, bag bag) int {
	total := 0
	for sub, n := range bag.contains {
		total += n
		for _, b := range bags {
			if b.color == sub {
				total += (n * totalBags(bags, b))
			}
		}
	}
	return total
...
func parseBags(in string) []bag {
	var bags []bag
	for _, line := range strings.Split(in, "\n") {
		line = line[:len(line)-1] // strip trailing .
		bagName := strings.Split(line, " bags contain ")[0]
		rules := strings.Split(line, " bags contain ")[1]
		if rules == "no other bags" {
			continue
		}
		bag := bag{color: bagName, contains: map[string]int{}}
		for _, rule := range strings.Split(rules, ", ") {
			rule = strings.TrimSuffix(rule, " bags")
			rule = strings.TrimSuffix(rule, " bag")
			numStr := strings.Fields(rule)[0]
			num, err := strconv.Atoi(numStr)
			if err != nil {
				panic(err)
			}
			rest := strings.TrimSpace(strings.TrimPrefix(rule, numStr))
			bag.contains[rest] = num
		}
...
func canHold(bags []bag, bag bag, color string) bool {
...
var data = `shiny gold bags contain 2 dark red bags.
...
var fulldata = `striped green bags contain 5 posh indigo bags.
