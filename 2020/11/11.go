...
func main() {
	...
	for _, row := range strings.Split(data, "\n") {
		seats = append(seats, []byte(row))
	}
	for {
		next := iterate(seats)
		same := true
		for i := range seats {
			if string(next[i]) != string(seats[i]) {
				same = false
				break
			}
		}
...
func iterate(seats [][]byte) [][]byte {
	var nextIter [][]byte
	for _, row := range seats {
		var nextRow []byte
		nextRow = append(nextRow, row...)
		nextIter = append(nextIter, nextRow)
	}
	for i := 0; i < len(seats); i++ {
		for j := 0; j < len(seats[i]); j++ {
			switch seats[i][j] {
			case empty:
				if numAdjacent(seats, i, j) == 0 {
					nextIter[i][j] = occupied
				}
			case floor:
			case occupied:
				if numAdjacent(seats, i, j) >= 5 {
					nextIter[i][j] = empty
				}
			}
		}
...
func numAdjacent(seats [][]byte, i, j int) int {
	res := 0
	for _, xdir := range []int{-1, 0, 1} {
		for _, ydir := range []int{-1, 0, 1} {
			...
			if first(seats, i, j, xdir, ydir) == occupied {
				res++
			}
		}
	}
...
func first(seats [][]byte, row, col, xdir, ydir int) byte {
	for {
		row = row + ydir
		col = col + xdir
		if row >= len(seats) || row < 0 {
			return floor
		}
		if col >= len(seats[row]) || col < 0 {
			return floor
		}
...
func iterate_partone(seats [][]byte) [][]byte {
	var nextIter [][]byte
	for _, row := range seats {
		var nextRow []byte
		nextRow = append(nextRow, row...)
	...
	for i := 0; i < len(seats); i++ {
		for j := 0; j < len(seats[i]); j++ {
			switch seats[i][j] {
			case empty:
				if numAdjacent_partone(seats, i, j) == 0 {
					nextIter[i][j] = occupied
				}
			case floor:
			case occupied:
				if numAdjacent_partone(seats, i, j) >= 4 {
					nextIter[i][j] = empty
				}
			}
		}
...
func numAdjacent_partone(seats [][]byte, i, j int) int {
	numAdjacent := 0
	if i > 0 {
		if j > 0 && seats[i-1][j-1] == occupied {
			numAdjacent++
		}
		if seats[i-1][j] == occupied {
			numAdjacent++
		}
		if j < len(seats[i-1])-1 && seats[i-1][j+1] == occupied {
			numAdjacent++
		}
	}
	...
	if j < len(seats[i])-1 && seats[i][j+1] == occupied {
		numAdjacent++
	}
	if i < len(seats)-1 {
		if j > 0 && seats[i+1][j-1] == occupied {
...
var data = `LLLLLLLL.LLLLLLLLL.LL.LLLLLLL.LLLLL.LLLLLLLL.LLLLL.LLLL.LLLLL.LLLLLLLLLLLLLLLLLLLLLLL.LLLLLLLLLL
