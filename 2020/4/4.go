		...
		if valid {
			for _, fieldName := range reqFields {
				...
				if !validateField(fieldName, fieldValue) {
					fmt.Println("Invalid:", fieldName, "val:", fieldValue)
					valid = false
					break
				} else {
					fmt.Println("Valid:", fieldName, "val:", fieldValue)
				}
			}
		}
		if valid {
...
func validateField(fieldName, fieldValue string) bool {
	...
	case "byr":
		...
		num, err := strconv.Atoi(fieldValue)
		if err != nil {
			return false
		}
	...
	case "hgt":
		...
		switch {
		case strings.HasSuffix(fieldValue, "in"):
			num, err := strconv.Atoi(fieldValue[:2])
			if err != nil {
				return false
			}
			if num < 59 || num > 76 {
		...
		case strings.HasSuffix(fieldValue, "cm"):
			num, err := strconv.Atoi(fieldValue[:3])
			if err != nil {
				return false
			}
			if num < 150 || num > 193 {
	...
	case "hcl":
		match, _ := regexp.MatchString("^#[0-9a-f]{6}$", fieldValue)
		if !match {
			return false
		}
	case "ecl":
	...
	case "pid":
		match, _ := regexp.MatchString("^[0-9]{9}$", fieldValue)
		if !match {
			return false
		}
	case "cid":
...
var data = `pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
var fulldata = `hcl:5d90f0 cid:270 ecl:#66dc9c hgt:62cm byr:1945 pid:63201172 eyr:2026
