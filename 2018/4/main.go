...
func main_part1() {
	...
	for _, line := range lines {
		...
		if strings.Contains(desc, "#") {
			guardId = strings.Split(strings.Split(desc, "#")[1], " ")[0]
		} else if strings.Contains(desc, "falls asleep") {
			sleepTime = t
		} else if strings.Contains(desc, "wakes up") {
			for m := sleepTime; m.Before(t); m = m.Add(time.Minute) {
				sleepMinutes[guardId] = append(sleepMinutes[guardId], m.Minute())
			}
		}
	}
	max := 0
	maxGuard := ""
	for guardId, minutes := range sleepMinutes {
		if len(minutes) > max {
			max = len(minutes)
			maxGuard = guardId
		}
	...
	for _, minute := range sleepMinutes[maxGuard] {
		minuteCounts[minute]++
	}
	max = 0
	maxK := 0
	for k, v := range minuteCounts {
		if v > max {
			max = v
			maxK = k
		}
	}
	fmt.Println(maxGuard, maxK)
}
...
func main() {
	...
	for guardId, minutes := range sleepMinutes {
		...
		for m, count := range minuteCounts {
			if count > max {
				max = count
				maxMinute = m
				maxGuardId = guardId
